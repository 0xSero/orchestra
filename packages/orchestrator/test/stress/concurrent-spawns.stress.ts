...
describe("Concurrent Spawn Race Condition", () => {
  ...
  test("BUGGY: concurrent spawns create multiple workers (demonstrates bug)", async () => {
    ...
    console.log(
      `[RACE-TEST] Buggy spawner: concurrency=${concurrency}, actualSpawns=${spawner.actualSpawnCount}, uniquePids=${spawner.uniquePids.length}`,
    );
    ...
    if (hasRaceCondition) {
      console.log(
        `[RACE-TEST] Race condition detected: ${spawner.actualSpawnCount} spawns for 1 profile`,
      );
  ...
  test("FIXED: concurrent spawns should deduplicate to single worker", async () => {
    ...
    console.log(
      `[RACE-TEST] Fixed spawner: concurrency=${concurrency}, actualSpawns=${spawner.actualSpawnCount}, uniquePids=${spawner.uniquePids.length}`,
    );
  ...
  test("STRESS: high concurrency reliably triggers race condition in buggy version", async () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      if (spawner.actualSpawnCount > 1) {
        totalRaceConditions++;
        console.log(
          `[STRESS] Iteration ${i + 1}: Race condition - ${spawner.actualSpawnCount} spawns`,
        );
      }
    }

    console.log(
      `[STRESS] Race conditions triggered: ${totalRaceConditions}/${iterations} iterations`,
    );
  ...
  test("deduplication should be faster than multiple spawns", async () => {
    ...
    console.log(
      `[TIMING] Buggy: ${buggyTime.toFixed(2)}ms for ${buggySpawns} spawns`,
    );
    console.log(
      `[TIMING] Fixed: ${fixedTime.toFixed(2)}ms for ${fixedSpawns} spawns`,
    );
    ...
    if (buggySpawns > 1) {
      ...
      console.log(
        `[TIMING] Buggy had ${buggySpawns - 1} extra spawns due to race condition`,
      );
    }
  });
  ...
  test("errors propagate to all concurrent callers", async () => {
    ...
    class FailingMockSpawner extends MockSpawner {
      ...
      async spawnWorkerFailing(
        ...
        let resolveSpawn!: (instance: WorkerInstance) => void;
        let rejectSpawn!: (error: Error) => void;
        this.inFlightPromise = new Promise<WorkerInstance>(
          (resolve, reject) => {
            resolveSpawn = resolve;
            rejectSpawn = reject;
          },
...
describe("Real Spawner Integration (if available)", () => {
  ...
  test("integration: real spawner deduplication", async () => {
    const { spawnWorker, stopWorker } = await import(
      "../../src/workers/spawner"
    );
