...
export function startEventPublisher(showToast: ToastFn): () => void {
  ...
  return onOrchestratorEvent((event) => {
    if (event.type === "orchestra.workflow.started") {
      ...
      const name = typeof data.workflowName === "string" ? data.workflowName : workflowId;
    ...
    if (event.type === "orchestra.workflow.completed") {
      ...
      const msg = status === "error" ? `Workflow "${name}" completed with errors` : `Workflow "${name}" completed`;
    ...
    if (event.type === "orchestra.error") {
      const data = event.data as Record<string, unknown>;
      const message = typeof data.message === "string" ? data.message : "Orchestrator error";
