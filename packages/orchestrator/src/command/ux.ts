...
import { listDeviceRegistry } from "../core/worker-pool";
...
type UxTools = {
  orchestratorHelp: ToolDefinition;
  enableDocsPassthrough: ToolDefinition;
  setPassthroughMode: ToolDefinition;
  clearPassthroughMode: ToolDefinition;
  orchestratorStart: ToolDefinition;
  orchestratorDashboard: ToolDefinition;
  orchestratorOutput: ToolDefinition;
  orchestratorResults: ToolDefinition;
  orchestratorDeviceRegistry: ToolDefinition;
  orchestratorDemo: ToolDefinition;
  orchestratorTodoView: ToolDefinition;
  macosKeybindsFix: ToolDefinition;
};

export function createUxTools(context: OrchestratorContext): UxTools {
  const workerPool = context.workerPool;
  const { autofillProfileModels } = createProfileTools(context);

  const getClient = () => context.client;
  const getDirectory = () => context.directory;
  const getProfiles = () => context.profiles;
  const getSpawnDefaults = () => context.spawnDefaults;
  const getDefaultListFormat = () => context.defaultListFormat;

async function ensureModelsConfigured(input: {
  client: ReturnType<typeof getClient>;
  openModels: boolean;
  showToast: boolean;
}): Promise<boolean> {
  const { client, openModels, showToast } = input;
  if (!client) return false;
  try {
    const { providers } = await fetchProviders(client, getDirectory());
    const scoped = filterProviders(providers, "configured");
    const models = flattenProviders(scoped);
  ...
  if (openModels) {
    void client.tui.openModels().catch(() => {});
  }
  if (showToast) {
    void client.tui.showToast({
      body: { message: "No configured models found. Opened model picker.", variant: "warning" },
    }).catch(() => {});
  }
  return false;
}

const orchestratorHelp: ToolDefinition = tool({
...
const enableDocsPassthrough: ToolDefinition = tool({
  description:
...
const setPassthroughMode: ToolDefinition = tool({
  description:
...
const clearPassthroughMode: ToolDefinition = tool({
...
const orchestratorStart: ToolDefinition = tool({
  description:
  ...
  args: {
    ...
    openHelp: tool.schema.boolean().optional().describe("Open the help panel (default: false)"),
    openModels: tool.schema.boolean().optional().describe("Open model picker if none configured (default: true)"),
  ...
  async execute(args, ctx: ToolContext) {
    ...
    const showToast = args.showToast ?? true;
    const openHelp = args.openHelp ?? false;
    const openModels = args.openModels ?? true;
    const enablePassthrough = args.enablePassthrough ?? true;
    const seedDocs = args.seedDocs ?? true;
    const smokeTest = args.smokeTest ?? true;

    if (openHelp) {
      void client.tui.openHelp().catch(() => {});
    }

    const modelsReady = await ensureModelsConfigured({ client, openModels, showToast });
    if (!modelsReady) {
      return "No configured models found. Configure a model and re-run `orchestrator.start`.";
    }
    ...
    if (!instance) {
      ...
      if (showToast) {
        void client.tui
          .showToast({
            body: { message: `Failed to start docs worker: ${lastError ?? "unknown"}`, variant: "error" },
          })
          .catch(() => {});
      }
    ...
    if (smokeTest) {
      ...
      try {
        ping = await sendToWorker("docs", "Reply with exactly: pong", {
          timeout: 120_000,
...
const orchestratorDashboard: ToolDefinition = tool({
...
const orchestratorOutput: ToolDefinition = tool({
  description:
...
const orchestratorResults: ToolDefinition = tool({
...
const orchestratorDeviceRegistry: ToolDefinition = tool({
...
const orchestratorDemo: ToolDefinition = tool({
  ...
  args: {
    ...
    openHelp: tool.schema.boolean().optional().describe("Open the help panel (default: true)"),
    openModels: tool.schema.boolean().optional().describe("Open model picker if none configured (default: true)"),
  },
  async execute(args, ctx: ToolContext) {
    ...
    const openHelp = args.openHelp ?? true;
    const openModels = args.openModels ?? true;
    ...
    if (client && openModels) {
      const ok = await ensureModelsConfigured({ client, openModels, showToast });
      if (!ok) {
        sections.push("", "## Models", "", "No configured models found. Configure a model and re-run the demo.");
      }
    }
...
const orchestratorTodoView: ToolDefinition = tool({
  description:
...
const macosKeybindsFix: ToolDefinition = tool({
  description:
...
const defaultTools: UxTools = createUxTools(getOrchestratorContext());

export const orchestratorHelp: ToolDefinition = defaultTools.orchestratorHelp;
export const enableDocsPassthrough: ToolDefinition = defaultTools.enableDocsPassthrough;
export const setPassthroughMode: ToolDefinition = defaultTools.setPassthroughMode;
export const clearPassthroughMode: ToolDefinition = defaultTools.clearPassthroughMode;
export const orchestratorStart: ToolDefinition = defaultTools.orchestratorStart;
export const orchestratorDashboard: ToolDefinition = defaultTools.orchestratorDashboard;
export const orchestratorOutput: ToolDefinition = defaultTools.orchestratorOutput;
export const orchestratorResults: ToolDefinition = defaultTools.orchestratorResults;
export const orchestratorDeviceRegistry: ToolDefinition = defaultTools.orchestratorDeviceRegistry;
export const orchestratorDemo: ToolDefinition = defaultTools.orchestratorDemo;
export const orchestratorTodoView: ToolDefinition = defaultTools.orchestratorTodoView;
export const macosKeybindsFix: ToolDefinition = defaultTools.macosKeybindsFix;
