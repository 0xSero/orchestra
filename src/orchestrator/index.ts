...
export const createOrchestrator: Factory<OrchestratorConfig, OrchestratorDeps, OrchestratorService> = ({
  ...
  const ensureWorker = async (input: { workerId: string; reason: "manual" | "on-demand" }) => {
    ...
    if (input.reason === "on-demand" && Array.isArray(config.spawnOnDemand)) {
      if (!config.spawnOnDemand.includes(input.workerId)) {
        throw new Error(`Worker "${input.workerId}" is not enabled for on-demand spawn.`);
      }
    }
  ...
  const start = async () => {
    ...
    for (const id of spawnIds) {
      ...
      await deps.workers.spawnById(id).catch(() => {});
    }
  };
