  ...
  <aside class="w-72 border-r border-border overflow-hidden flex flex-col bg-card/30">
    ...
    <div class="flex-1 overflow-auto scrollbar-thin">
      <For each={props.agents}>
        {(agent) => (
          <button
            class={`session-item w-full text-left ${props.selectedWorkerId === agent.id ? "selected" : ""}`}
            onClick={() => props.onSelectWorker(agent.id)}
          >
            <div class="session-item-header">
              <div class="flex items-center gap-2 min-w-0 flex-1">
                <span class="session-item-title">{agent.frontmatter.name ?? agent.id}</span>
              </div>
            </div>
            <div class="session-item-meta mt-1">
              <span class="truncate">{agent.frontmatter.model}</span>
            </div>
          </button>
        )}
      </For>
      ...
      <Show when={props.agents.length === 0}>
        <div class="empty-state py-12">
          <p class="empty-state-title">No agents loaded</p>
          <p class="empty-state-description">Add agent profiles to manage overrides.</p>
        </div>
      </Show>
    </div>
  </aside>
