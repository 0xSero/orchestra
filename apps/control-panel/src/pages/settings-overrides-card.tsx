...
export const SettingsOverridesCard: Component<SettingsOverridesCardProps> = (props) => {
  ...
  createEffect(() => {
    ...
    const baseTemp = agent?.frontmatter.temperature ?? 0.7;
  ...
  const handleResetOverrides = async () => {
    if (!props.agent) return;
    await props.onReset(props.agent.id);
  };
            ...
            <div class="space-y-4">
              <div class="grid gap-4 md:grid-cols-2">
                <label class="flex flex-col gap-2 text-xs text-muted-foreground">
                  ...
                  <Input
                    value={modelOverride()}
                    placeholder={agent().frontmatter.model ?? "auto"}
              ...
              <div class="grid gap-4 md:grid-cols-2">
                <div class="space-y-2">
                  ...
                  <div class="text-xs text-muted-foreground">Default: {agent().frontmatter.temperature ?? 0.7}</div>
                </div>
